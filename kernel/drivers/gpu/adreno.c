...
#ifdef CONFIG_MOBED
	if(earom_gpu_stats_enabled()){
		if(!earom_gpu_stats_initialized()){
			earom_gpu_stats_init((int)(device->pwrctrl.num_pwrlevels));
		}else{
			earom_gpu_stats_update((int)device->pwrctrl.active_pwrlevel,
					stats->busy_time,
					adreno_ticks_to_us(stats->ram_time+stats->ram_wait,
						kgsl_pwrctrl_active_freq(pwr)));
		}
	}
#endif
